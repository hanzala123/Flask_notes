...
def view_notes():
    ...
    return render_template('items/dashboard.html', data=data,username=WEB_USER)
...
@session_required
def save_note():
    myNote = {'title':request.form['title'],'text': request.form['text']}
    # Create
    ...
    if request.form['id']:
        ...
        for i in range(0,len(my_notes)):
            if my_notes[i]['title'] == request.form['id']:
                my_notes[i] = {'title':request.form['title'],'text': request.form['text']}
                break
