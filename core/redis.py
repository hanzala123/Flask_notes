...
class RedisManager:
  ...
  def update(self, key1, key2, value):
    tmp = self.get_json(key1)
    tmp[key2] = value
    self.store_json(key1, tmp)
  
  def remove_element(self, key, delElement):
    data = rds.get_json(key)
    data.pop(delElement)
    if data:
      rds.store_json(key, data)
    else:
      rds.delete(key)
  ...
  def get_domains(self):
    domains = []
    data = self.r.get('domains')
    if data:
      try:
        domains = pickle.loads(data)
        #logger.info('domains: ' + str(domains))
      except:
        logger.error('Error retrieving domains')
  ...
  def get_json(self, key):
    ...
    if data:
      try:
        ...
        #logger.info('Getting json from ' + str(key))
      except:
        logger.error('Error retrieving json')
    return tmp


  def get_subdomain_count(self):
    try:
      subdomainData = self.get_json('subum')
      count = len(subdomainData[self.get_domains()[0]])
    except:
      count = 0
    return count
  ...
  def is_subum_active(self):
    tmp = self.get_json('subum')
    if tmp['finished']:
      return False
    return True
  ...
  def is_session_active(self): 
    if self.is_scan_active() or self.is_attack_active() or self.is_subum_active():
      return True
    return False
  ...
  def clear_session(self):
    ...
    self.r.delete('domains')
    tmp = self.get_json('subum')
    tmp['finished'] = True
    self.store_json('subum', tmp)
